...
import { ApiError, useAssets, useCampaign, useCreateCreative, useUpdateCampaign } from '@project/sdk'
...
export function CampaignCreateCreativePage() {
  ...
  const assets = assetsQuery.data?.pages.flatMap((page) => page.data) ?? []
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      setError(err instanceof ApiError || err instanceof Error ? err.message : 'Could not create creative')
    }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-muted-foreground">Creates a library creative and attaches it to this campaign.</p>
      ...
      {assets.length === 0 ? (
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <Input id="creative-name" value={name} onChange={(e) => setName(e.target.value)} required />
          </div>
          <AssetPicker assets={assets} selectedIds={assetIds} onToggle={toggleAsset} />
          {error ? <p className="text-sm text-destructive">{error}</p> : null}
          <div className="flex gap-2">
            <Button type="submit" size="sm" disabled={createCreative.isPending || updateCampaign.isPending}>
              Create and attach
            </Button>
