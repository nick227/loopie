...
describe('calendar', () => {
  ...
  it('mirrors a Lead next action into Calendar and auto-completes it when the activity is logged', async () => {
    ...
    // Stay inside the current local week: on Sunday, +1 day lands in next week and falls
    // out of getBoard's today/thisWeek buckets (week ends Monday 00:00). +1 hour stays visible.
  ...
  it('auto-completes CRM Calendar work when a real message is sent, not just when activity is logged manually', async () => {
    ...
    // Same week-boundary constraint as the mirror test above — +1 hour, not +2 days.
    const nextActionAt = new Date(Date.now() + 60 * 60 * 1000).toISOString()
    await app.inject({
      method: 'PATCH',
      url: `/leads/${lead.id}`,
