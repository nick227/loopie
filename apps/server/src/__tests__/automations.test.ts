  ...
  it('runs CRUD lifecycle', async (ctx) => {
    ...
    try {
      if (!'/automations'.includes('{') || createdIds['automations']) {
        ...
        if (rescreateAutomation.statusCode === 201 && rescreateAutomation.json().data?.id)
          createdIds['automations'] = rescreateAutomation.json().data.id
        if (rescreateAutomation.statusCode !== 201) {
          console.error(
            'createAutomation failed with ' + rescreateAutomation.statusCode,
            rescreateAutomation.json().message || rescreateAutomation.json(),
          )
        }
    ...
    try {
      if (!'/automations/{automationId}/pause'.includes('{') || createdIds['automations']) {
        const respauseAutomationAuth = await app.inject({
          method: 'POST',
    ...
    try {
      if (!'/automations/{automationId}/pause'.includes('{') || createdIds['automations']) {
        ...
        if (respauseAutomation.statusCode === 201 && respauseAutomation.json().data?.id)
          createdIds['automations'] = respauseAutomation.json().data.id
        if (respauseAutomation.statusCode !== 200) {
          console.error(
            'pauseAutomation failed with ' + respauseAutomation.statusCode,
            respauseAutomation.json().message || respauseAutomation.json(),
          )
        }
    ...
    try {
      if (!'/automations/{automationId}/resume'.includes('{') || createdIds['automations']) {
        const resresumeAutomationAuth = await app.inject({
          method: 'POST',
    ...
    try {
      if (!'/automations/{automationId}/resume'.includes('{') || createdIds['automations']) {
        ...
        if (resresumeAutomation.statusCode === 201 && resresumeAutomation.json().data?.id)
          createdIds['automations'] = resresumeAutomation.json().data.id
        if (resresumeAutomation.statusCode !== 200) {
          console.error(
            'resumeAutomation failed with ' + resresumeAutomation.statusCode,
            resresumeAutomation.json().message || resresumeAutomation.json(),
          )
        }
    ...
    try {
      if (!'/automations'.includes('{') || createdIds['automations']) {
        ...
        if (reslistAutomations.statusCode !== 200) {
          console.error(
            'listAutomations failed with ' + reslistAutomations.statusCode,
            reslistAutomations.json().message || reslistAutomations.json(),
          )
        }
    ...
    try {
      if (!'/automations/{automationId}'.includes('{') || createdIds['automations']) {
        const resgetAutomationAuth = await app.inject({
          method: 'GET',
    ...
    try {
      if (!'/automations/{automationId}'.includes('{') || createdIds['automations']) {
        ...
        if (resgetAutomation.statusCode !== 200) {
          console.error(
            'getAutomation failed with ' + resgetAutomation.statusCode,
            resgetAutomation.json().message || resgetAutomation.json(),
          )
        }
    ...
    try {
      if (!'/automations/{automationId}/logs'.includes('{') || createdIds['automations']) {
        const reslistAutomationLogsAuth = await app.inject({
          method: 'GET',
    ...
    try {
      if (!'/automations/{automationId}/logs'.includes('{') || createdIds['automations']) {
        ...
        if (reslistAutomationLogs.statusCode !== 200) {
          console.error(
            'listAutomationLogs failed with ' + reslistAutomationLogs.statusCode,
            reslistAutomationLogs.json().message || reslistAutomationLogs.json(),
          )
        }
    ...
    try {
      if (!'/automations/{automationId}'.includes('{') || createdIds['automations']) {
        const resupdateAutomationAuth = await app.inject({
          method: 'PATCH',
          url: `/automations/${createdIds['automations'] || '00000000-0000-0000-0000-000000000001'}`,
        })
    ...
    try {
      if (!'/automations/{automationId}'.includes('{') || createdIds['automations']) {
        ...
        if (resupdateAutomation.statusCode !== 200) {
          console.error(
            'updateAutomation failed with ' + resupdateAutomation.statusCode,
            resupdateAutomation.json().message || resupdateAutomation.json(),
          )
        }
