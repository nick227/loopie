  ...
  it('runs CRUD lifecycle', async (ctx) => {
    ...
    try {
      if (!'/messages/{messageId}/send'.includes('{') || createdIds['messages']) {
        const ressendMessageAuth = await app.inject({
          method: 'POST',
    ...
    try {
      if (!'/messages/{messageId}/send'.includes('{') || createdIds['messages']) {
        ...
        if (ressendMessage.statusCode === 201 && ressendMessage.json().data?.id)
          createdIds['messages'] = ressendMessage.json().data.id
        if (ressendMessage.statusCode !== 200) {
          console.error(
            'sendMessage failed with ' + ressendMessage.statusCode,
            ressendMessage.json().message || ressendMessage.json(),
          )
        }
    ...
    try {
      if (!'/messages/{messageId}/test-send'.includes('{') || createdIds['messages']) {
        const restestSendMessageAuth = await app.inject({
          method: 'POST',
    ...
    try {
      if (!'/messages/{messageId}/test-send'.includes('{') || createdIds['messages']) {
        ...
        if (restestSendMessage.statusCode === 201 && restestSendMessage.json().data?.id)
          createdIds['messages'] = restestSendMessage.json().data.id
        if (restestSendMessage.statusCode !== 200) {
          console.error(
            'testSendMessage failed with ' + restestSendMessage.statusCode,
            restestSendMessage.json().message || restestSendMessage.json(),
          )
        }
    ...
    try {
      if (!'/messages'.includes('{') || createdIds['messages']) {
        ...
        if (reslistMessages.statusCode !== 200) {
          console.error(
            'listMessages failed with ' + reslistMessages.statusCode,
            reslistMessages.json().message || reslistMessages.json(),
          )
        }
    ...
    try {
      if (!'/messages/{messageId}'.includes('{') || createdIds['messages']) {
        const resgetMessageAuth = await app.inject({
          method: 'GET',
    ...
    try {
      if (!'/messages/{messageId}'.includes('{') || createdIds['messages']) {
        ...
        if (resgetMessage.statusCode !== 200) {
          console.error(
            'getMessage failed with ' + resgetMessage.statusCode,
            resgetMessage.json().message || resgetMessage.json(),
          )
        }
    ...
    try {
      if (!'/messages/{messageId}/performance'.includes('{') || createdIds['messages']) {
        const resgetMessagePerformanceAuth = await app.inject({
          method: 'GET',
    ...
    try {
      if (!'/messages/{messageId}/performance'.includes('{') || createdIds['messages']) {
        ...
        if (resgetMessagePerformance.statusCode !== 200) {
          console.error(
            'getMessagePerformance failed with ' + resgetMessagePerformance.statusCode,
            resgetMessagePerformance.json().message || resgetMessagePerformance.json(),
          )
        }
    ...
    try {
      if (!'/messages/{messageId}'.includes('{') || createdIds['messages']) {
        const resupdateMessageAuth = await app.inject({
          method: 'PATCH',
          url: `/messages/${createdIds['messages'] || '00000000-0000-0000-0000-000000000001'}`,
        })
    ...
    try {
      if (!'/messages/{messageId}'.includes('{') || createdIds['messages']) {
        ...
        if (resupdateMessage.statusCode !== 200) {
          console.error(
            'updateMessage failed with ' + resupdateMessage.statusCode,
            resupdateMessage.json().message || resupdateMessage.json(),
          )
        }
    ...
    try {
      if (!'/messages/{messageId}'.includes('{') || createdIds['messages']) {
        const resdeleteMessageAuth = await app.inject({
          method: 'DELETE',
          url: `/messages/${createdIds['messages'] || '00000000-0000-0000-0000-000000000001'}`,
        })
    ...
    try {
      if (!'/messages/{messageId}'.includes('{') || createdIds['messages']) {
        ...
        if (resdeleteMessage.statusCode !== 200) {
          console.error(
            'deleteMessage failed with ' + resdeleteMessage.statusCode,
            resdeleteMessage.json().message || resdeleteMessage.json(),
          )
        }
