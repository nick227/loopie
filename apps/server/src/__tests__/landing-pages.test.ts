  ...
  it('runs CRUD lifecycle', async (ctx) => {
    ...
    try {
      if (!'/landing-pages/{landingPageId}/publish'.includes('{') || createdIds['landing-pages']) {
        const respublishLandingPageAuth = await app.inject({
          method: 'POST',
    ...
    try {
      if (!'/landing-pages/{landingPageId}/publish'.includes('{') || createdIds['landing-pages']) {
        ...
        if (respublishLandingPage.statusCode === 201 && respublishLandingPage.json().data?.id)
          createdIds['landing-pages'] = respublishLandingPage.json().data.id
        if (respublishLandingPage.statusCode !== 201) {
          console.error(
            'publishLandingPage failed with ' + respublishLandingPage.statusCode,
            respublishLandingPage.json().message || respublishLandingPage.json(),
          )
        }
    ...
    try {
      if (
        !'/landing-pages/{landingPageId}/form-start'.includes('{') ||
        createdIds['landing-pages']
      ) {
        ...
        if (
          resrecordLandingPageFormStart.statusCode === 201 &&
          resrecordLandingPageFormStart.json().data?.id
        )
          createdIds['landing-pages'] = resrecordLandingPageFormStart.json().data.id
        if (resrecordLandingPageFormStart.statusCode !== 200) {
          console.error(
            'recordLandingPageFormStart failed with ' + resrecordLandingPageFormStart.statusCode,
            resrecordLandingPageFormStart.json().message || resrecordLandingPageFormStart.json(),
          )
        }
        ...
        await validateResponse(
          'recordLandingPageFormStart',
          200,
          resrecordLandingPageFormStart.json(),
        )
      }
    } catch (e: any) {
    ...
    try {
      if (!'/landing-page-templates'.includes('{') || createdIds['landing-page-templates']) {
        const reslistLandingPageTemplatesAuth = await app.inject({
          method: 'GET',
    ...
    try {
      if (!'/landing-page-templates'.includes('{') || createdIds['landing-page-templates']) {
        ...
        if (reslistLandingPageTemplates.statusCode !== 200) {
          console.error(
            'listLandingPageTemplates failed with ' + reslistLandingPageTemplates.statusCode,
            reslistLandingPageTemplates.json().message || reslistLandingPageTemplates.json(),
          )
        }
    ...
    try {
      if (
        !'/landing-page-templates/{templateId}'.includes('{') ||
        createdIds['landing-page-templates']
      ) {
        const resgetLandingPageTemplateAuth = await app.inject({
          method: 'GET',
    ...
    try {
      if (
        ...
        if (resgetLandingPageTemplate.statusCode !== 200) {
          console.error(
            'getLandingPageTemplate failed with ' + resgetLandingPageTemplate.statusCode,
            resgetLandingPageTemplate.json().message || resgetLandingPageTemplate.json(),
          )
        }
    ...
    try {
      if (!'/landing-pages'.includes('{') || createdIds['landing-pages']) {
        ...
        if (reslistLandingPages.statusCode !== 200) {
          console.error(
            'listLandingPages failed with ' + reslistLandingPages.statusCode,
            reslistLandingPages.json().message || reslistLandingPages.json(),
          )
        }
    ...
    try {
      if (!'/landing-pages/{landingPageId}'.includes('{') || createdIds['landing-pages']) {
        const resgetLandingPageAuth = await app.inject({
          method: 'GET',
    ...
    try {
      if (!'/landing-pages/{landingPageId}'.includes('{') || createdIds['landing-pages']) {
        ...
        if (resgetLandingPage.statusCode !== 200) {
          console.error(
            'getLandingPage failed with ' + resgetLandingPage.statusCode,
            resgetLandingPage.json().message || resgetLandingPage.json(),
          )
        }
    ...
    try {
      if (!'/landing-pages/{landingPageId}/versions'.includes('{') || createdIds['landing-pages']) {
        const reslistLandingPageVersionsAuth = await app.inject({
          method: 'GET',
    ...
    try {
      if (!'/landing-pages/{landingPageId}/versions'.includes('{') || createdIds['landing-pages']) {
        ...
        if (reslistLandingPageVersions.statusCode !== 200) {
          console.error(
            'listLandingPageVersions failed with ' + reslistLandingPageVersions.statusCode,
            reslistLandingPageVersions.json().message || reslistLandingPageVersions.json(),
          )
        }
    ...
    // previewLandingPage - auth check
    try {
      if (!'/landing-pages/{landingPageId}/preview'.includes('{') || createdIds['landing-pages']) {
        const respreviewLandingPageAuth = await app.inject({
          method: 'GET',
          url: `/landing-pages/${createdIds['landing-pages'] || '00000000-0000-0000-0000-000000000001'}/preview`,
        })
        expect(respreviewLandingPageAuth.statusCode).toBe(401)
      }
    } catch (e: any) {
      errors.push(new Error('previewLandingPage auth failed: ' + e.message))
    }

    try {
      if (!'/landing-pages/{landingPageId}/preview'.includes('{') || createdIds['landing-pages']) {
        const respreviewLandingPage = await app.inject({
          method: 'GET',
          url: `/landing-pages/${createdIds['landing-pages'] || '00000000-0000-0000-0000-000000000001'}/preview`,
          headers: asAuth(testUserId),
          // payload: {},
        })
        if (respreviewLandingPage.statusCode !== 200) {
          console.error(
            'previewLandingPage failed with ' + respreviewLandingPage.statusCode,
            respreviewLandingPage.json().message || respreviewLandingPage.json(),
          )
        }
        expect(respreviewLandingPage.statusCode).toBe(200)
        await validateResponse('previewLandingPage', 200, respreviewLandingPage.json())
      }
    } catch (e: any) {
      errors.push(new Error('previewLandingPage failed: ' + e.message))
    }
    ...
    try {
      if (!'/landing-pages/{landingPageId}/export'.includes('{') || createdIds['landing-pages']) {
        const resexportLandingPageAuth = await app.inject({
          method: 'GET',
    ...
    try {
      if (!'/landing-pages/{landingPageId}/export'.includes('{') || createdIds['landing-pages']) {
        ...
        if (resexportLandingPage.statusCode !== 200) {
          console.error(
            'exportLandingPage failed with ' + resexportLandingPage.statusCode,
            resexportLandingPage.json().message || resexportLandingPage.json(),
          )
        }
    ...
    try {
      if (
        !'/landing-pages/{landingPageId}/performance'.includes('{') ||
        createdIds['landing-pages']
      ) {
        const resgetLandingPagePerformanceAuth = await app.inject({
          method: 'GET',
    ...
    try {
      if (
        ...
        if (resgetLandingPagePerformance.statusCode !== 200) {
          console.error(
            'getLandingPagePerformance failed with ' + resgetLandingPagePerformance.statusCode,
            resgetLandingPagePerformance.json().message || resgetLandingPagePerformance.json(),
          )
        }
        ...
        await validateResponse(
          'getLandingPagePerformance',
          200,
          resgetLandingPagePerformance.json(),
        )
      }
    } catch (e: any) {
    ...
    try {
      if (!'/p/{slug}'.includes('{') || createdIds['p']) {
        ...
        if (resservePublishedLandingPage.statusCode !== 200) {
          console.error(
            'servePublishedLandingPage failed with ' + resservePublishedLandingPage.statusCode,
            resservePublishedLandingPage.json().message || resservePublishedLandingPage.json(),
          )
        }
        ...
        await validateResponse(
          'servePublishedLandingPage',
          200,
          resservePublishedLandingPage.json(),
        )
      }
    } catch (e: any) {
    ...
    try {
      if (!'/landing-pages/{landingPageId}'.includes('{') || createdIds['landing-pages']) {
        const resupdateLandingPageAuth = await app.inject({
          method: 'PATCH',
          url: `/landing-pages/${createdIds['landing-pages'] || '00000000-0000-0000-0000-000000000001'}`,
        })
    ...
    try {
      if (!'/landing-pages/{landingPageId}'.includes('{') || createdIds['landing-pages']) {
        ...
        if (resupdateLandingPage.statusCode !== 200) {
          console.error(
            'updateLandingPage failed with ' + resupdateLandingPage.statusCode,
            resupdateLandingPage.json().message || resupdateLandingPage.json(),
          )
        }
    ...
    // replaceLandingPageAdSlots - auth check
    try {
      if (!'/landing-pages/{landingPageId}/ad-slots'.includes('{') || createdIds['landing-pages']) {
        const resreplaceLandingPageAdSlotsAuth = await app.inject({
          method: 'PUT',
          url: `/landing-pages/${createdIds['landing-pages'] || '00000000-0000-0000-0000-000000000001'}/ad-slots`,
        })
        expect(resreplaceLandingPageAdSlotsAuth.statusCode).toBe(401)
      }
    } catch (e: any) {
      errors.push(new Error('replaceLandingPageAdSlots auth failed: ' + e.message))
    }

    try {
      if (!'/landing-pages/{landingPageId}/ad-slots'.includes('{') || createdIds['landing-pages']) {
        const resreplaceLandingPageAdSlots = await app.inject({
          method: 'PUT',
          url: `/landing-pages/${createdIds['landing-pages'] || '00000000-0000-0000-0000-000000000001'}/ad-slots`,
          headers: asAuth(testUserId),
          payload: {
        ...
        if (resreplaceLandingPageAdSlots.statusCode !== 200) {
          console.error(
            'replaceLandingPageAdSlots failed with ' + resreplaceLandingPageAdSlots.statusCode,
            resreplaceLandingPageAdSlots.json().message || resreplaceLandingPageAdSlots.json(),
          )
        }
        expect(resreplaceLandingPageAdSlots.statusCode).toBe(200)
        await validateResponse(
          'replaceLandingPageAdSlots',
          200,
          resreplaceLandingPageAdSlots.json(),
        )
      }
    } catch (e: any) {
      errors.push(new Error('replaceLandingPageAdSlots failed: ' + e.message))
    }
    ...
    try {
      if (!'/landing-pages/{landingPageId}'.includes('{') || createdIds['landing-pages']) {
        const resdeleteLandingPageAuth = await app.inject({
          method: 'DELETE',
          url: `/landing-pages/${createdIds['landing-pages'] || '00000000-0000-0000-0000-000000000001'}`,
        })
    ...
    try {
      if (!'/landing-pages/{landingPageId}'.includes('{') || createdIds['landing-pages']) {
        ...
        if (resdeleteLandingPage.statusCode !== 200) {
          console.error(
            'deleteLandingPage failed with ' + resdeleteLandingPage.statusCode,
            resdeleteLandingPage.json().message || resdeleteLandingPage.json(),
          )
        }
