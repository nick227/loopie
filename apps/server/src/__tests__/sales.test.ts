  ...
  it('runs CRUD lifecycle', async (ctx) => {
    ...
    try {
      if (!'/sales/{saleId}/reverse'.includes('{') || createdIds['sales']) {
        const resreverseSaleAuth = await app.inject({
          method: 'POST',
    ...
    try {
      if (!'/sales/{saleId}/reverse'.includes('{') || createdIds['sales']) {
        ...
        if (resreverseSale.statusCode === 201 && resreverseSale.json().data?.id)
          createdIds['sales'] = resreverseSale.json().data.id
        if (resreverseSale.statusCode !== 200) {
          console.error(
            'reverseSale failed with ' + resreverseSale.statusCode,
            resreverseSale.json().message || resreverseSale.json(),
          )
        }
    ...
    try {
      if (!'/sales'.includes('{') || createdIds['sales']) {
        ...
        if (reslistSales.statusCode !== 200) {
          console.error(
            'listSales failed with ' + reslistSales.statusCode,
            reslistSales.json().message || reslistSales.json(),
          )
        }
    ...
    try {
      if (!'/sales/{saleId}'.includes('{') || createdIds['sales']) {
        const resgetSaleAuth = await app.inject({
          method: 'GET',
    ...
    try {
      if (!'/sales/{saleId}'.includes('{') || createdIds['sales']) {
        ...
        if (resgetSale.statusCode !== 200) {
          console.error(
            'getSale failed with ' + resgetSale.statusCode,
            resgetSale.json().message || resgetSale.json(),
          )
        }
