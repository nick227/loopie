...
describe('contacts API', () => {
    ...
    try {
      if (!'/contacts'.includes('{') || createdIds['contacts']) {
        ...
        if (reslistContacts.statusCode !== 200) {
          console.error('listContacts failed with ' + reslistContacts.statusCode, reslistContacts.json().message || reslistContacts.json())
        }
  ...
  "name": "test_string"
        ...
        if (rescreateContact.statusCode === 201 && rescreateContact.json().data?.id) createdIds['contacts'] = rescreateContact.json().data.id
        if (rescreateContact.statusCode !== 201) {
          console.error('createContact failed with ' + rescreateContact.statusCode, rescreateContact.json().message || rescreateContact.json())
        }
    ...
    try {
      if (!'/contacts/import'.includes('{') || createdIds['contacts']) {
  ...
  "contacts": [
    {
      "name": "test_string"
        ...
        if (resimportContacts.statusCode === 201 && resimportContacts.json().data?.id) createdIds['contacts'] = resimportContacts.json().data.id
        if (resimportContacts.statusCode !== 200) {
          console.error('importContacts failed with ' + resimportContacts.statusCode, resimportContacts.json().message || resimportContacts.json())
        }
    ...
    try {
      if (!'/contacts/{contactId}'.includes('{') || createdIds['contacts']) {
        const resgetContactAuth = await app.inject({ method: 'GET', url: `/contacts/${createdIds['contacts'] || '00000000-0000-0000-0000-000000000001'}` })
    ...
    try {
      if (!'/contacts/{contactId}'.includes('{') || createdIds['contacts']) {
        ...
        if (resgetContact.statusCode !== 200) {
          console.error('getContact failed with ' + resgetContact.statusCode, resgetContact.json().message || resgetContact.json())
        }
    ...
    try {
      if (!'/contacts/{contactId}'.includes('{') || createdIds['contacts']) {
        const resupdateContactAuth = await app.inject({ method: 'PATCH', url: `/contacts/${createdIds['contacts'] || '00000000-0000-0000-0000-000000000001'}` })
    ...
    try {
      if (!'/contacts/{contactId}'.includes('{') || createdIds['contacts']) {
        ...
        if (resupdateContact.statusCode !== 200) {
          console.error('updateContact failed with ' + resupdateContact.statusCode, resupdateContact.json().message || resupdateContact.json())
        }
    ...
    try {
      if (!'/contacts/{contactId}'.includes('{') || createdIds['contacts']) {
        const resdeleteContactAuth = await app.inject({ method: 'DELETE', url: `/contacts/${createdIds['contacts'] || '00000000-0000-0000-0000-000000000001'}` })
    ...
    try {
      if (!'/contacts/{contactId}'.includes('{') || createdIds['contacts']) {
        ...
        if (resdeleteContact.statusCode !== 200) {
          console.error('deleteContact failed with ' + resdeleteContact.statusCode, resdeleteContact.json().message || resdeleteContact.json())
        }
    ...
    try {
      if (!'/contacts/{contactId}/interactions'.includes('{') || createdIds['contacts']) {
        const reslistContactInteractionsAuth = await app.inject({ method: 'GET', url: `/contacts/${createdIds['contacts'] || '00000000-0000-0000-0000-000000000001'}/interactions` })
    ...
    try {
      if (!'/contacts/{contactId}/interactions'.includes('{') || createdIds['contacts']) {
        ...
        if (reslistContactInteractions.statusCode !== 200) {
          console.error('listContactInteractions failed with ' + reslistContactInteractions.statusCode, reslistContactInteractions.json().message || reslistContactInteractions.json())
        }
